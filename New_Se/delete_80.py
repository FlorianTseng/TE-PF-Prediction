...
# 读取CSV文件
df = pd.read_csv('New_Te&Se_unique.csv')

# 函数：检查化合物是否包含原子序数大于80的元素
def contains_high_atomic_number(formula):
    import re
    elements = re.findall(r'[A-Z][a-z]?', formula)
    return any(atomic_numbers.get(element, 0) > 80 for element in elements)

# 过滤数据
filtered_df = df[~df['formula'].apply(contains_high_atomic_number)]

# 保存过滤后的数据到新的CSV文件
filtered_df.to_csv('Filtered_New_Te&Se_unique.csv', index=False)

print('过滤完成，结果已保存到Filtered_New_Te&Se_unique.csv文件中。')
