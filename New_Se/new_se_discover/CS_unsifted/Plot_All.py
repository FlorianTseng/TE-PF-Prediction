...
for tg in target:
    ...
    train_mape = mean_absolute_percentage_error(train_targets, train_features)
    train_r2 = r2_score(train_targets, train_features)

    test_mape = mean_absolute_percentage_error(test_targets, test_features)
    ...
    ax.text(0.05, 0.80, f'Train MAPE: {train_mape:.3f}', transform=ax.transAxes, fontsize=16,
            fontname='Times New Roman')
    ax.text(0.05, 0.75, f'Train R-squared: {train_r2:.3f}', transform=ax.transAxes, fontsize=16,
            fontname='Times New Roman')
    ax.text(0.05, 0.7, f'Test MAPE: {test_mape:.3f}', transform=ax.transAxes, fontsize=16,
            fontname='Times New Roman')
